...
class KinematicsNode(Node): # MODIFY NAME
    ...
    def callback_compute(self, request: ComputeIK.Request, response: ComputeIK.Response):
        ...
        while within_range == False and trial < max_trial:
            theta_sol, converge, iterations = self.inverse_kinematics_space(self.s_list, self.M, T_sd, initial_guess)
            # No convergence
            if converge == False:
                initial_guess = np.array([np.random.uniform(low, high) for low, high in random_ranges])
                trial += 1
        ...
        response.converged = converge

        print("Result:", theta_sol)
        print("Converged:", converge, "after", iterations, "iterations")
        print("RESULT POSE:")
    ...
    def matrix_log6(self, T):
        logT = logm(T)
        if np.max(np.abs(logT.imag)) > 1e-8:
            print("Warning: Significant imaginary part in matrix_log6!")
        return logT.real
