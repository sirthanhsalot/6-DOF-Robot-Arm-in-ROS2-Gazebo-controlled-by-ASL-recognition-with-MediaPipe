...
if __name__ == "__main__":
    ...
    offset = [1024, 1024, 0, 1024+70, 1024]
    converge = False
    within_range = False
    ...
    while within_range == False and trial < max_trial:
        ...
        for i in range(DoF):
            integer_values[i] = int(np.round(4096 * theta_sol[i] / (2*np.pi))) + offset[i]
            if integer_values[i] >= 4096:
                integer_values[i] -= 4096
        # Check within range
        if all(integer_values[i] <= constraints[i] for i in range(DoF)) == True and integer_values[3] >= 70:
            within_range = True
            trial += 1
            break
        initial_guess = np.array([np.random.uniform(low, high) for low, high in random_ranges])
