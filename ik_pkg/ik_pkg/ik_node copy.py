...
class KinematicsNode(Node): # MODIFY NAME
    ...
    def callback_compute(self, request: ComputeIK.Request, response: ComputeIK.Response):
        ...
        # print("INITIAL ANGLES:", theta_0)    
        #home configuration M
        # M = np.array([[0,0,1,21.09],
        #             [-1,0,0,0],
        #             [0,-1,0,31.6],
        #             [0,0,0,1]])
        
        # #Screw axes 
        # s_list = np.array([[0, 0   , 0     , 1   , 0     , 1   ],
        #                 [0, -1  , -1    , 0   , -1    , 0   ],
        #                 [1, 0   ,  0    , 0   , 0     , 0   ], 
        #                 [0, 16.5, 31.6  , 0   , 31.6  , 0   ],
        #                 [0, 0   , 0     , 31.6, 0     , 31.6],
        ...
        # foward kinematics to calculate current position
        # T_current = self.fk_poe(self.s_list, theta_0, self.M)
        # print("current pose:")
        # print(T_current)
        # self.get_logger().info("Current position: ", T_current)
        # T_b = fk_poeB(b_list, theta, M)
        # print("Homogeneous Transformation Matrix:")
        # print(T_current)
        # print("Homogeneous Transformation Matrix in base frame:")
        # print(T_b)
        ...
        while within_range == False and trial < max_trial:
            theta_sol, success, iters = self.inverse_kinematics_space(self.s_list, self.M, T_sd, theta_0, max_iters=100)
            # No convergence
            if success == False:
                theta_0 = [np.random.uniform(-np.pi, np.pi) for _ in range(len(theta_0))]
                trial += 1
                continue
            ...
            # Check within range
            if all(-np.pi <= theta_sol[i] <= np.pi for i in range(len(theta_sol))):
                within_range = True
                trial += 1
                break
            theta_0 = [np.random.uniform(-np.pi, np.pi) for _ in range(len(theta_0))]
        ...
        # Round angles to 5 decimal places and convert to float32
        ...
        print("Result:")
        print(theta_sol)
        print("Converged:", success, "after", iters, "iterations")
        print("RESULT POSE:")
